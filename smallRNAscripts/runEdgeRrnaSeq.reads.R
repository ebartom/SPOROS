...
if (runMDS==1){
   ...
   newnames <- colnames(counts)
   colnames(countData) <- gsub(".justReads.uniqCounts.txt$","",newnames)
   if(filterOff==0){
...
#    print(head(subdata))
    ...
    print(dim(subdata))
    if (filterOff == 1){
        print("Not filtering data based on minimum cpms.")
        dge <- DGEList(subdata, group=grp) # calc lib.sizes
    }else{
        print ("Filtering data based on minimum cpms.")
...
#    df <- cbind(as.data.frame(geneData[foo,]),RPKM,lrt$table,adj.p=p.adjust(lrt$table$PValue,method="BH"))
    ...
    df <- cbind(CPM,lrt$table,adj.p=p.adjust(lrt$table$PValue,method="BH"))
